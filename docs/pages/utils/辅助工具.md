...
> [!WARNING|label:有限的使用]
> 需要先进入官方群组进行注册，然后通过[官方机器人账号](https://t.me/CreazyRabbitBot)获取 `授权许可文件` 才能使用，关于如何进入群组详见官方频道
...
- #### 部署方法
  ...
  > 面板有两个版本，这里推荐最新的 **FastRabbit**，具体区别详见[官方介绍](https://t.me/Rabbit_one/275)，若部署其它版本只需更改镜像名  
  > 若同时部署多个版本注意映射端口不要冲突，可公用同一配置文件即无需更改映射目录  
  > 不通版本对应镜像名称：`FastRabbit ➜ fastrabbit`，`MadRabbit ➜ madrabbit`

  ```bash
  docker run -dit \
  --name fastrabbit `# 容器名` \
  --hostname fastrabbit `# 主机名` \
  --network bridge `# 容器网络类型` \
  --restart always `# 开机自启` \
  -p 6790:1234 `# 端口映射，"主机端口:容器端口"` \
  -v /opt/Rabbit/Config:/Rabbit/Config `# 配置文件的主机挂载目录` \
  --privileged=true \
  ht944/fastrabbit:latest
  ```
  > [!WARNING]
  > 注意不可以更改 `:` 右边的内容，否则会报错，可更改默认映射端口号以此设置面板访问端口  
  > 若容器启动后不能访问网页或遇到其它错误请通过 `docker logs -f fastrabbit` 命令查看容器日志
...
- #### 更新方法
  ...
  - ##### 更新源码
    ...
    访问面板自动更新接口路径 `/api/update` 或进入到终端执行 `git pull`

  - ##### 更新镜像

    ```bash
    docker run --rm -v /var/run/docker.sock:/var/run/docker.sock containrrr/watchtower -c --run-once fastrabbit
    ```
