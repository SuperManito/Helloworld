...
- #### 客户操作指南
  ...
  > 对方提供给您的自助登录网址在以下的教程内容中简称为**面板**，注意请使用系统浏览器访问网址不要用微信打开面板

    <div>
    <details>

    <summary title="折叠代码块">:fa-brands fa-apple: 苹果</summary>
    ...
    <div>
    <details>
    <summary title="折叠代码块">:fa-brands fa-android: 安卓 及 <a><img src="src/img/icon/harmonyOS.png" width="12%" height="12%" style="vertical-align: -0.35em"></a> 鸿蒙</summary>

      > 请通过系统内置浏览器APP访问**面板**

      - ###### 1. 下载证书

        在面板点击 `安装证书` 按钮，此时提示下载一个名为 `ca.crt` 的文件，请操作下载至本地（如果您修改了文件的默认下载位置那么请牢记您配置的路径）

      - ###### 2. 安装证书

        在系统设置APP中搜索证书，点击进入 `从存储设备安装证书`，点击 `CA证书`，此时可能需要验证您手机的锁屏密码

        ![android2](../../src/img/cktool/android1.png ':size=60%')

        之后会进入一个浏览文件的界面让您选择证书的位置，点击左上角进入菜单，选择 `显示内部存储` 或您的设备型号相关字样进入到手机文件目录，在众多目录和文件中依次进入到 `Download` - `Browser` 浏览器下载文件默认存放目录，选择刚刚下载的 `ca.crt` 文件，点击 `确定` 安装证书

      - ###### 3. 配置代理并连接

        进入到系统设置APP的WLAN设置中，长按已连接的Wi-Fi，点击修改配置（如果没有就试试点按），进入高级配置页面后下滑至底部找到 `HTTP代理`，点击 `配置代理` - `手动`，进行如下两步操作：
        ...
        ![android3](../../src/img/cktool/android2.png)
      ...
      - ##### 5. 关闭代理
        ...
        进入到系统设置APP的WLAN设置中，长按已连接的Wi-Fi，点击修改配置（如果没有就试试点按），进入高级配置页面后下滑至底部找到 `HTTP代理`，点击 `配置代理` - `无`
