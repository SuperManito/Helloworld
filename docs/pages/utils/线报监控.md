...
线报客户端监控插件插件依托 [PagerMaid-Pyro（人形机器人）](https://github.com/TeamPGM/PagerMaid-Pyro)平台运行，因此需要先安装该服务  
该平台功能强大后期维护插件较为便利，还有其它功能丰富的官方插件[（详见此处）](https://telegra.ph/PagerMaid-Plugins-11-27)可以使用，自行体验

  - ### 注意事项

    > [!TIP|label:友情提示]
    > 给自己的人形部署在网络条件较好的环境里（建议使用境外VPS），断网就监控不到线报了哦~  

    > [!ATTENTION|label:警告]
    > 不建议除 DC5（新加坡数据中心）以外的其它地区电报账号使用，封号几率较大并且大概率无法找回，具体解决方法详见下方[常见问题](pages/utils/线报监控?id=常见问题)  
    > 人形相当于电报账号的客户端，然后通过脚本去实现各种功能，滥用插件或其它同类工具可能会被官方视为非正常用户行为，因而导致封号或被踢下线
  ...
  - ### 脚本一键安装
    ...
    > [!TIP|label:友情提示]
    > 安装过程中若遇到问题请前往[官方群组](https://t.me/+pCLtkzj4Yo41ZTM9)进行咨询，请认真查看命令行输出的提示，安装完成后建议开启**数据持久化**
...
## 安装与使用
  ...
  - ### 配置插件
    ...
    - 设置你的容器BOT信息（必填），大致位于第23行的 `USER_BOT` 变量，需修改默认的变量值，支持数字ID和用户名（具体格式见注释内容）

    - 设置你的运行账号配置（可选），大致位于第47行的代码，用法与指定账号参数相同

    - 设置你的脚本全局代理配置（可选），大致位于第61行的代码，用法与网络代理参数相同

    - 设置你的推送渠道配置（可选），大致位于第69行的代码，用于指定推送渠道以便于满足多容器应用需求
  ...
  - ### 安装/更新插件
    ...
    将你编辑好的插件发送至电报客户端的收藏夹，回复 `,apt install` 进行安装（注意不要修改脚本名）
  ...
  - ### 使用方法
    ...
    - #### 监控屏蔽
      ...
      取消屏蔽只需要将 `set` 改为 `unset`，该功能对应前面的标记使用方法
  ...
  - ### 拉取监控依赖脚本库
    ...
    ```
    如果该库已经作为了主库使用，那么直接设置软链接即可，无需重复拉库，容器内执行 `ln -sf /jd/scripts /jd/own/KingRan_KR`
...
## 常见问题
  ...
  - ### 关于电报账号所在数据中心（DataCenter） <!-- {docsify-ignore} -->
    ...
    - #### 电报账号在 _DC5_ 但被官方误封了怎么办？ <!-- {docsify-ignore} -->
      ...
      官方可能不会及时发送处理邮件，期间请尝试登录验证并耐心等待，极大概率会找回，后面可能就不会被再次误封了

  - ### 关于人形无法使用监控插件 <!-- {docsify-ignore} -->
  ...
  - ### 监控到的线报消息被发送消息至收藏夹（Saved Message） <!-- {docsify-ignore} -->

    `USER_BOT` 变量定义的是你的容器Bot而不是你自己的电报账号，请认真理解线报监控工作原理

  - ### 监控不到线报/监控插件没有工作 <!-- {docsify-ignore} -->

    监控插件不推送代表 PagerMaid-Pyro 人形 Bot 挂了，或者你的插件没有正确配置并启用，电报机器人（容器Bot）不执行则代表电报机器人挂了

    服务挂了最常见的原因就是网络问题，请自行尝试提升你的魔法，不要来问问问！！！

  - ### 关于人形报错 `400 PEER_ID_INVALID` <!-- {docsify-ignore} -->

    一般使用数字ID才会出现该问题，建议换为用户名
