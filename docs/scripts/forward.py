...
async def filters(text):
    # 初始化一些变量
    is_lzkj = is_lzkjdz = is_cjhy = is_cjhydz = is_txzj = False # 过滤标记
    ...
    # 5. 在 try 作用域下代码报错会自动向你的bot发送错误信息，但是不一定会影响监控的正常运行
    ...
    try:
        ...
        if "task env edit " in text:
            ...
            # 店铺抽奖 · 超级无线/超级会员
            if script == 'jd_luck_draw.js':
                text = text + LZKJ_RUNS if is_lzkj else text + CJHY_RUNS

            # 加购有礼 · 超级无线/超级会员
            elif script == 'jd_wxCollectionActivity.js':
                text += ADDCARTS_RUNS

            # 关注店铺有礼 · 超级无线/超级会员
            elif script == 'jd_wxShopFollowActivity.js':
                text = text + LZKJ_RUNS if is_lzkj else text + CJHY_RUNS

            # 店铺礼包 · 超级无线/超级会员
            elif script == 'jd_wxShopGift.js':
                text = text + LZKJ_RUNS if is_lzkj else text + CJHY_RUNS

            # 知识超人 · 超级无线/超级会员
            elif script == 'jd_wxKnowledgeActivity.js':
                text = text + LZKJ_RUNS if is_lzkj else text + CJHY_RUNS
            ...
            # 分享有礼 · 超级无线
            elif script == 'jd_wxShareActivity.js':
                text = text
                is_lzkj = is_lzkjdz = True # 用于屏蔽标记判断（勿动）
            ...
            # 加购有礼 · 收藏大师
            elif script == 'jd_txzj_cart_item.js':
                text += ADDCARTS_RUNS

            # 关注商品有礼 · 收藏大师
            elif script == 'jd_txzj_collect_item.js':
                text += TXZJ_RUNS

            # 关注店铺有礼 · 收藏大师
            elif script == 'jd_txzj_collect_shop.js':
                text += TXZJ_RUNS

            # 幸运抽奖 · 收藏大师
            elif script == 'jd_txzj_lottery.js':
                text += TXZJ_RUNS
            ...
            # 大牌集合 · 京耕
            # elif script == 'jd_opencardDPLHTY.js':
            #     text = text

            # joyjd抽奖机
            # elif script == 'jd_lottery.js':
            #     text = text

            # joyjd开卡
            # elif script == 'jd_joyopen.js':
            #     text = text
        ...
        ## 监控屏蔽（勿动）
        if (is_lzkj) and (sqlite.get("forwardMark.disable_lzkj")):
            return False
        if (is_lzkjdz) and (sqlite.get("forwardMark.disable_lzkjdz")):
            return False
        if (is_cjhy) and (sqlite.get("forwardMark.disable_cjhy")):
            return False
        if (is_cjhydz) and (sqlite.get("forwardMark.disable_cjhydz")):
            return False
        if (is_txzj) and (sqlite.get("forwardMark.disable_txzj")):
            return False
...
async def forward(message: Message):
    ...
    if ID_BOT == '1234567890':
        await message.edit("⚠ 请先在此脚本中定义你的 BOT id 后才能使用哦~")
        await sleep(5)
...
async def forward_message(message: Message):
    try:
        ...
        await log(f"forward 监控到新消息：{str(text)}")
        if not results:
            return
